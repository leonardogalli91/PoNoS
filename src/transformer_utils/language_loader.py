...
class LMOrderedIterator(object):
    def __init__(
        ...
        data = data.narrow(0, 0, int(self.n_step * bsz))
...
def ptb_loader(
    batch_size, workspace_dir, device, tgt_len, drop_last=False, full_batch=False, shuffle=False
):
    ...
    if not os.path.isdir(datadir):
        subprocess.run(
            [
                "sh",
                "src/transformer_utils/get_ptb.sh",
...
def wikitext2_loader(
    ...
    if not os.path.isdir(datadir):
        subprocess.run(
            [
                "sh",
                "src/transformer_utils/get_wikitext2.sh",
                os.path.abspath(os.path.join(workspace_dir, "datasets")),
            ],
            check=True,
            cwd=cwd,
